...
class MessageValidatorMixin(object):
    def __init__(self, *args, **kwargs):
        ...
        # set to true initially because first message drives the settings
        self.accept_unknown_fields = True
    ...
    def init_settings(self, *args, **kwargs):
        ...
        self.accept_unknown_fields = self.settings.getboolean(
            'AcceptUnknownFields', fallback=False)
        self.send_time_tolerance = datetime.timedelta(
            minutes=self.settings.get('SendingTimeTolerance', fallback=2))
        self.validate_comp_ids = self.settings.getboolean(
            'ValidateCompIDs', fallback=True)
        self.validate_req_fields = self.settings.getboolean(
            'ValidateRequiredFields', fallback=True)

    def register_admin_messages(self, *args, **kwargs):
        super().register_admin_messages(*args, **kwargs)
        self.register_admin_callback(
            None, self.on_first_message, priority=CallbackRegistrar.CALLBACK_PRIORITY.FIRST, one_time=True)
        if self.validate_comp_ids:
            self.register_admin_callback(
                None, self.on_validate_message, priority=CallbackRegistrar.CALLBACK_PRIORITY.FIRST + CallbackRegistrar.CALLBACK_PRIORITY.AFTER)
        if self.send_time_tolerance > datetime.timedelta(minutes=0):
            self.register_admin_callback(
                None, self.on_validate_sendtime, priority=CallbackRegistrar.CALLBACK_PRIORITY.FIRST + CallbackRegistrar.CALLBACK_PRIORITY.AFTER)
        if self.validate_req_fields:
            self.register_admin_callback(
                None, self.on_validate_req_fields, priority=CallbackRegistrar.CALLBACK_PRIORITY.FIRST + CallbackRegistrar.CALLBACK_PRIORITY.AFTER)
    ...
    async def parse_message(self, *args, **kwargs):
        try:
            ...
            if not self.accept_unknown_fields and len(missed_fields) > 0:
                ...
                raise fix_errors.FIXInvalidMessageFieldError(
                    msg.Header.MsgSeqNum, msg._msgtype, ref_tag, "Invalid tag number", self.message_lib.fields.SessionRejectReason.ENUM_INVALID_TAG_NUMBER)
            ...
            self.send_reject(curr_seq, e.RefMsgType, e.RefTagID,
                             e.Text, e.SessionRejectReason)
            return None, None, None
    ...
    def on_validate_message(self, session_name, msg):
        if msg.Header.BeginString != self.__BeginString:
            ...
            raise fix_errors.FIXInvalidMessageError(
                error, wait_interval=2, send_test_msg=False)
    ...
    def on_validate_sendtime(self, session_name, msg):
        send_time = datetime.datetime.strptime(
            msg.Header.SendingTime, self.time_format)
